...
public class ExcelDeal {
    public static void main(String[] args) {
       //  String arr[] = {"aaa", "xxx"};
        ...
        List<File> files= getAllFile("E:\\www\\product\\jiafang");
        for(File f:files){
           ...
           // String filename = "product-import-template.xlsx";
            ...
            if(data_list.size() > 0){
                createExcel(filename, title_list, data_list);
            }
    ...
    public static void createExcel(String filename, List title_list, List data_list){
        try (Workbook workbook = new XSSFWorkbook();
             FileOutputStream out = new FileOutputStream("E:\\www\\product\\jiafang-out\\" + filename)) {
            ...
            int cols = title_list.size();
            for(int i = 0; i < cols; i++){
                Cell cell0i = row0.createCell(i);
                String v = (String) title_list.get(i);
                cell0i.setCellValue(v);
            }

            for(int k = 0; k < data_list.size(); k++){
                Row row = (Row)data_list.get(k);
                Row row_new = sheet0.createRow(k + 1);
                for(int t = 0; t < cols; t++){
                    Cell cell_obj = row.getCell(t);
                    if(cell_obj != null){
                        cell_obj.setCellType(CellType.STRING);
                        String str = cell_obj.getStringCellValue();

                        Cell cell_kt = row_new.createCell(t);
                        cell_kt.setCellValue(str);
                    }
                }
            }
